...
@router.get("/patient-registration/", response_model=schemas.PaginatedResponse[schemas.PatientRegistration])
...
def delete_patient_registration(id: int, db: Session = Depends(get_db)):
...
def search_patients(hospid: int = Query(...), db: Session = Depends(get_db)):
    mf_ids = db.query(models.PatientInfo.mf_id).filter(models.PatientInfo.hospital_id == hospid).all()
    mf_ids = [mfid for (mfid,) in mf_ids]  # unpack tuple

    results = []
    for mfid in mf_ids:
        pinfo = db.query(models.PatientInfo).filter(models.PatientInfo.mf_id == mfid).first()
        preg = db.query(models.PatientRegistration).filter(models.PatientRegistration.mf_id == mfid).first()
        if pinfo and preg:
            results.append({
...
@router.get("/patients/", response_model=schemas.PaginatedResponse[schemas.PatientInfo])
def get_patients(
    hospid: Optional[int] = Query(None),
    ...
    return schemas.PaginatedResponse(total=total, limit=limit, offset=offset, items=items)
